 ...
 */
   ...
   */
		...
		build( data, resourcePath, onProgress, onError ) {
			...
			// Preserve PMX material morph data (type=0 group, type=8 material)
			const materialMorphData = {};
			for ( let i = 0; i < data.metadata.morphCount; i ++ ) {

				const morph = data.morphs[ i ];
				if ( morph.type === 8 ) {

					// Direct material morph
					const ops = [];
					for ( let j = 0; j < morph.elements.length; j ++ ) {
						...
						ops.push( {
							materialIndex: el.index,
							opType: el.type,          // 0=MUL, 1=ADD
							diffuse: [ el.diffuse[ 0 ], el.diffuse[ 1 ], el.diffuse[ 2 ], el.diffuse[ 3 ] ],
							specular: [ el.specular[ 0 ], el.specular[ 1 ], el.specular[ 2 ] ],
							shininess: el.shininess,
							ambient: [ el.ambient[ 0 ], el.ambient[ 1 ], el.ambient[ 2 ] ],
							edgeColor: [ el.edgeColor[ 0 ], el.edgeColor[ 1 ], el.edgeColor[ 2 ], el.edgeColor[ 3 ] ],
							edgeSize: el.edgeSize
						} );
					...
					if ( ops.length > 0 ) {
						...
						materialMorphData[ morph.name ] = { type: 8, elements: ops };

					}

				} else if ( morph.type === 0 ) {

					// Group morph: check if contains material morph children
					const children = [];
					for ( let j = 0; j < morph.elements.length; j ++ ) {

						const childMorph = data.morphs[ morph.elements[ j ].index ];
						if ( childMorph.type === 8 ) {

							children.push( {
								name: childMorph.name,
								ratio: morph.elements[ j ].ratio
							} );
					...
					if ( children.length > 0 ) {
						...
						materialMorphData[ morph.name ] = { type: 0, children: children };
			...
			mesh.userData.materialMorphData = materialMorphData;
		...
		build( data, geometry
      	 ...
      	 */
				...
				if ( data.metadata.format === 'pmd' ) {
					...
					// PMX S影 (Self-Shadow) 标记: bit2=cast, bit3=receive
					params.userData.selfShadowCast    = ( material.flag & 0x04 ) !== 0;
					params.userData.selfShadowReceive = ( material.flag & 0x08 ) !== 0;
