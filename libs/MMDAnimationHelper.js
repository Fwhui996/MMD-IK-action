			...
			for ( let i = 0, il = iks.length; i < il; i ++ ) {
				...
				for ( let j = 0, jl = links.length; j < jl; j ++ ) {
					...
					if ( physicsEnabled === true ) {
						...
						// Keep all IK links enabled when physics is on.
						// IK solves bone positions first, then physics applies forces.
						// Disabling IK for any bone causes rigid bodies to read stale transforms.
						link.enabled = true;
